...
import {
  useAccount, usePublicClient, useWriteContract,
  useConnect, useDisconnect, useSwitchChain, useChainId,
} from 'wagmi';
...
export default function PayPage() {
  ...
  }, [connectAsync, connectors, currentChainId, isConnected, switchChainAsync]);

  // ▼ useCallback so useEffect can safely depend on it
  ...
  useEffect(() => { load(); }, [load]); // ✅ satisfies eslint rule
  ...
  useEffect(() => {
    ...
    const unwatch = publicClient.watchContractEvent({
      ...
      abi: erc20Abi,
      eventName: 'Transfer',
      // ▼ type the logs to avoid `any`
      onLogs: async (logs: readonly { args?: { value?: bigint; to?: `0x${string}` } ; transactionHash: `0x${string}` }[]) => {
        const match = logs.find((l) => l.args?.to === paylink.to && l.args?.value === amountAtomic);
        if (match) {
    ...
    return () => { unwatch?.(); setWatching(false); };
  }, [publicClient, paylink, amountAtomic, load]);

  if (loading || !paylink) return <p>Loading…</p>;

  async function onPayClick() {
    if (!paylink) { alert('Pay link not loaded yet.'); return; }
    if (!publicClient) { alert('Public client not ready.'); return; }
    const p = paylink;
    ...
    try {
      ...
      const txHash = await writeContractAsync({
        ...
        args: [p.to, BigInt(p.amountMicros)],
      });
  ...
  return (
    <main style={{ display: 'grid', gap: 12 }}>
      ...
      <div>
        Token: <a href={`${ENV.EXPLORER_BASE}/token/${paylink.tokenAddress}`} target="_blank" rel="noreferrer">
